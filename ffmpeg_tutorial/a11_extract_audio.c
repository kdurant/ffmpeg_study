...
/**
* @brief 从视频文件中提取出音频(mp4文件测试成功，wmv测试失败)
*
* @param in_filename
* @param out_filename
*/
void extractAudio(char *in_filename, char *out_filename)
{
    ...
    if(ret < 0)
    {
        av_log(NULL, AV_LOG_ERROR, "cannot open file, error message = %d\n", ret);
        return;
    }

    FILE *dst_fd = fopen(out_filename, "wb");
    if(!dst_fd)
    {
    ...
    av_log(NULL, AV_LOG_INFO, "Audio Stream Index = %d", audio_index);
...
int main(void)
{
    extractAudio("http://vfx.mtime.cn/Video/2019/03/17/mp4/190317150237409904.mp4", "output1.acc");
